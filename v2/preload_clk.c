...
{   
    ...
    fprintf(logFile,"%lf %lf fopen %s %s = %p \n",called_time+(times*2160),exec_time,pathname,mode,ret);
...
int fclose(FILE *stream)
{
    ...
    fprintf(logFile,"%lf %lf fclose %p = %d\n",called_time+(times*2160),exec_time,stream,ret);
...
int fgetc(FILE *__stream)
{
    ...
    fprintf(logFile,"%lf %lf fgetc %p = %d\n",called_time+(times*2160),exec_time,__stream,ret);
...
int fputc(int character, FILE *stream)
{
    ...
    fprintf(logFile,"%lf %lf fputc %d %p = %d\n",called_time+(times*2160),exec_time,character,stream,ret);
...
size_t fread( void * ptr, size_t size, size_t count, FILE * stream )
{
    ...
    fprintf(logFile,"%lf %lf fread %zu %zu %p = %zu\n",called_time+(times*2160),exec_time,size,count,stream,ret);
...
char * fgets ( char * str, int num, FILE * stream )
{
    ...
    fprintf(logFile,"%lf %lf fgets %s %d %p = %s\n",called_time+(times*2160),exec_time,str,num,stream,ret);
...
int fputs ( const char * str, FILE * stream )
{
    ...
    fprintf(logFile,"%lf %lf fputs %s %p = %d\n",called_time+(times*2160),exec_time,str,stream,ret);
...
int fseek ( FILE * stream, long int offset, int origin )
{
    ...
    fprintf(logFile,"%lf %lf fseek %p %ld %d = %d\n",called_time+(times*2160),exec_time,stream,offset,origin,ret);
...
int open(const char *pathname, int flags, ...)
{
    ...
    fprintf(logFile,"%lf %lf open %s %d = %d\n",called_time+(times*2160),exec_time,pathname,flags,ret);
...
ssize_t read(int fd, void *buf, size_t count)
{
    ...
    fprintf(logFile,"%lf %lf read %d %p %zu = %zu\n",called_time+(times*2160),exec_time,fd,buf,count,ret);
...
ssize_t pread(int fd, void *buf, size_t count, off_t offset)
{
    ...
    fprintf(logFile,"%lf %lf pread %d %p %zu %ld = %zu\n",called_time+(times*2160),exec_time,fd,buf,count,offset,ret);
...
int close(int fd)
{
    ...
    fprintf(logFile,"%lf %lf close %d = %d\n",called_time+(times*2160),exec_time,fd,ret);
